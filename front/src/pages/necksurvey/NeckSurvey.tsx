...
const NeckSurvey = () => {
    const question=['사무실에서 컴퓨터를 하루 6시간 이상 사용한다.','목을 뒤로 젖히면 아프다','자주 목과 어깨가 결리고 딱딱하게 굳는다.','등이 굽었다','목을 움직이면 각도에 따라 아프다', '잠버릇이 나쁘다는 말을 자주 듣는다','어떤 베개를 사용해도 편하지 않다.','팔을 올리고 자는 버릇이 있다.','사진을 보면 언제나 목이 기울어 있다.','목,어깨를 비틀면 똑똑 소리가 난다.']
    const navigate=useNavigate();
    const imgContents=[survey1,survey2,survey3,survey4,survey5,survey6,survey7,survey8,survey9,survey10]
    ...
    <S.SurveyContainer>
      ...
      <S.ContentBox>
        <S.TextBox>
          ...
          <S.SubTitle style={{fontWeight:400, marginTop:16}}>{question[page-1]}</S.SubTitle>
        </S.TextBox>
